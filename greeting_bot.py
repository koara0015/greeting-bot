...
async def on_message(message):
    ...
    if message.content.startswith('t!user'):
        ...
        # t!clear コマンド（特定ユーザーのメッセージ削除・管理者限定）
    if message.content.startswith('t!clear'):
        ...
        if len(parts) != 3 or not parts[1].isdigit() or not parts[2].isdigit():
            await message.channel.send("使い方：t!clear [ユーザーID] [削除するメッセージ数]")
            return
        ...
        if count <= 0 or count > 100:
            await message.channel.send("⚠️ 削除数は1～100の間で指定してください。")
            return

        target_user = message.guild.get_member(target_id)
        if not target_user:
            await message.channel.send("⚠️ 指定されたユーザーが見つかりません。")
            return

        try:
            deleted = await message.channel.purge(
                limit=200,  # 直近200件からフィルター
                check=lambda m: m.author.id == target_id,
                bulk=True
            )
            deleted = deleted[:count]  # 指定数に制限
            for m in deleted:
                await m.delete()

            embed = discord.Embed(
                description=f"🧹 **{target_user.display_name}** の **{len(deleted)}** 個のメッセージを削除しました。",
    ...
    # t!clean コマンド（直近のメッセージを削除・管理者限定）
    if message.content.startswith('t!clean'):
        if message.author.id != admin_id:
            await message.channel.send("⚠️ 権限がありません")
            return
        ...
        if len(parts) != 2 or not parts[1].isdigit():
            await message.channel.send("使い方：t!clean [削除するメッセージ数]")
            return
