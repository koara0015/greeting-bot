...
async def on_message(message):
    ...
    # t!ai コマンド（なんちゃってAI応答）
    if message.content.startswith('t!ai'):
        user_input = message.content[4:].strip()
        if not user_input:
            await message.channel.send("使い方：t!ai [話しかけたい内容]")
            return

        responses = [
            "それ、面白いね！もっと教えて！",
            "ちょっと何言ってるかわからないけど、楽しいね！",
            "うーん、それはどうなんだろうねぇ…？",
            "それ、ググったほうが早いかも！？",
            "なるほど、そういう考えもあるかもね！",
            "ぼくはAIだから、ちょっと感情とかはよくわからないけど…！",
            "へぇ〜、そんなことがあるんだ！初耳！",
        ]
    ...
    if message.content == 't!help':
        if message.author.id == admin_id:
            ...
            embed.add_field(name="🟢 t!ai [メッセージ]", value="なんちゃってAIが返事をします（誰でも可）", inline=False)
